...
// PROJECT IMPORT
import { categoryGroup } from "features/categoryGroup/categoryGroupSlice";
...
const CategoryGroupTable = ({
  ...
  const categoryGroups = categoryGroupState.data.list;
  const pagination = categoryGroupState.data.pagination;
  ...
  const renderTableHead = () => {
    ...
    return (
      <TableRow>
        ...
        <TableCell sx={styleCell}>Tên nhóm chuyên mục</TableCell>
  ...
  const renderTableBody = (item: CategoryGroupType, index: number) => {
    return (
  ...
  const handleStatus = (value: number, id: number | undefined) => {
    ...
    dispatch({
      type: "categoryGroup/updateStatus",
      payload: {
        id: id,
        params: item,
      },
  ...
  const renderStatus = (item: CategoryGroupType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: CategoryGroupType) => (
    <>
      <Button
        size="small"
        variant="outlined"
    ...
    <Box sx={{ position: "relative", pb: 2 }}>
      <TableContainer sx={{ overflow: "auto" }}>
        <Table>
          ...
          <TableBody>
            {categoryGroups?.map((item, index) => renderTableBody(item, index))}
          </TableBody>
        </Table>
      ...
      {categoryGroups?.length === 0 && <NoData />}
      {loading && <Loading />}
