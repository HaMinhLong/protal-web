...
const SupplierTable = ({
  ...
  const suppliers = supplierState.data.list;
  const pagination = supplierState.data.pagination;
  ...
  const renderTableBody = (item: SupplierType, index: number) => {
    return (
      <TableRow hover key={item.id}>
        ...
        <TableCell>{item?.supplierGroup?.name}</TableCell>
        <TableCell
          sx={{
            overflow: "hidden",
  ...
  const renderStatus = (item: SupplierType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: SupplierType) => (
    <>
      <Button
        size="small"
        variant="outlined"
    ...
    <Box sx={{ position: "relative", pb: 2 }}>
      <TableContainer sx={{ overflow: "auto" }}>
        <Table>
          ...
          <TableBody>
            {suppliers?.map((item, index) => renderTableBody(item, index))}
          </TableBody>
        </Table>
        {suppliers?.length > 0 && (
          <Pagination
