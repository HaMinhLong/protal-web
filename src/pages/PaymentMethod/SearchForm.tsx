...
// TYPES IMPORT
import { FilterPaymentMethod, PaymentMethodType } from "types/paymentMethod";
...
const SearchForm = ({ setDataEdit, setVisibleDrawer, setLoading }: Props) => {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      name: paymentMethodState?.filter?.name || "",
      status: paymentMethodState?.filter?.status || "",
    },
    onSubmit: (values) => {
      handleSearch(values);
    },
  ...
  const handleSearch = (values: FilterPaymentMethod) => {
    setLoading(true);
    const queryName: FilterPaymentMethod = {
