...
interface Props {
  dataEdit: PaymentMethodType;
  setDataEdit: Dispatch<SetStateAction<PaymentMethodType>>;
...
const PaymentMethodTable = ({
  ...
  const paymentMethods = paymentMethodState.data.list;
  const pagination = paymentMethodState.data.pagination;
  ...
  const renderTableBody = (item: PaymentMethodType, index: number) => {
    return (
  ...
  const renderStatus = (item: PaymentMethodType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: PaymentMethodType) => (
    <>
      <Button
        size="small"
        variant="outlined"
    ...
    <Box sx={{ position: "relative", pb: 2 }}>
      <TableContainer sx={{ overflow: "auto" }}>
        <Table>
          ...
          <TableBody>
            {paymentMethods?.map((item, index) => renderTableBody(item, index))}
          </TableBody>
        </Table>
      ...
      {paymentMethods?.length === 0 && <NoData />}
      {loading && <Loading />}
