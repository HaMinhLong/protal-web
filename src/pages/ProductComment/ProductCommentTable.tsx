...
// PROJECT IMPORT
import { productComment } from "features/productComment/productCommentSlice";
...
const ProductCommentTable = ({
  ...
  const productComments = productCommentState.data.list;
  const pagination = productCommentState.data.pagination;
  ...
  const renderTableHead = () => {
    ...
    return (
      <TableRow>
        ...
        <TableCell sx={styleCell}>Bình luận</TableCell>
  ...
  const renderTableBody = (item: ProductCommentType, index: number) => {
    return (
  ...
  const handleStatus = (value: number, id: number | undefined) => {
    ...
    dispatch({
      type: "productComment/updateStatus",
      payload: {
        id: id,
        params: item,
      },
  ...
  const renderStatus = (item: ProductCommentType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: ProductCommentType) => (
    <>
      <Button
        size="small"
        variant="outlined"
  ...
  return (
    <Box sx={{ position: "relative", pb: 2 }}>
      ...
      {productComments?.length === 0 && <NoData />}
      {loading && <Loading />}
