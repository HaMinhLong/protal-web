...
const ArticleTable = ({
  ...
  const articles = articleState.data.list;
  const pagination = articleState.data.pagination;
  ...
  const renderTableHead = () => {
    ...
    return (
      <TableRow>
        ...
        <TableCell sx={styleCell}>Tên tin tức</TableCell>
        <TableCell sx={styleCell}>Website</TableCell>
        <TableCell sx={styleCell}>Chuyên mục</TableCell>
        <TableCell sx={styleCell}>Tác giả</TableCell>
  ...
  const renderTableBody = (item: ArticleType, index: number) => {
    return (
  ...
  const renderStatus = (item: ArticleType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: ArticleType) => (
    <>
      <Button
        size="small"
        variant="outlined"
    ...
    <Box sx={{ position: "relative", pb: 2 }}>
      <TableContainer sx={{ overflow: "auto" }}>
        <Table>
          ...
          <TableBody>
            {articles?.map((item, index) => renderTableBody(item, index))}
          </TableBody>
        </Table>
        {articles?.length > 0 && (
          <Pagination
