...
// PROJECT IMPORT
import { producer } from "features/producer/producerSlice";
...
const ProducerTable = ({
  ...
  const producers = producerState.data.list;
  const pagination = producerState.data.pagination;
  ...
  const renderTableHead = () => {
    ...
    return (
      <TableRow>
        ...
        <TableCell sx={styleCell}>Tên nhà sản xuất</TableCell>
  ...
  const renderTableBody = (item: ProducerType, index: number) => {
    return (
      <TableRow hover key={item.id}>
        ...
        <TableCell>{item?.producerGroup?.name}</TableCell>
        <TableCell
          sx={{
            overflow: "hidden",
  ...
  const renderStatus = (item: ProducerType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: ProducerType) => (
    <>
      <Button
        size="small"
        variant="outlined"
    ...
    <Box sx={{ position: "relative", pb: 2 }}>
      <TableContainer sx={{ overflow: "auto" }}>
        <Table>
          ...
          <TableBody>
            {producers?.map((item, index) => renderTableBody(item, index))}
          </TableBody>
        </Table>
        {producers?.length > 0 && (
          <Pagination
