...
// PROJECT IMPORT
import { supplierGroup } from "features/supplierGroup/supplierGroupSlice";
...
const SupplierGroupTable = ({
  ...
  const supplierGroups = supplierGroupState.data.list;
  const pagination = supplierGroupState.data.pagination;
  ...
  const renderTableBody = (item: SupplierGroupType, index: number) => {
    return (
  ...
  const renderStatus = (item: SupplierGroupType) => {
    return (
      <SwitchStatus
        status={item?.status}
        id={item?.id}
  ...
  const renderMenuButton = (item: SupplierGroupType) => (
    <>
      <Button
        size="small"
        variant="outlined"
    ...
    <Box sx={{ position: "relative", pb: 2 }}>
      <TableContainer sx={{ overflow: "auto" }}>
        <Table>
          ...
          <TableBody>
            {supplierGroups?.map((item, index) => renderTableBody(item, index))}
          </TableBody>
        </Table>
      ...
      {supplierGroups?.length === 0 && <NoData />}
      {loading && <Loading />}
