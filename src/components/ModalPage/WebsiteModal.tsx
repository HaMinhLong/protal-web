...
const WebsiteModal = ({ visible, closeDrawer, dataEdit, getList }: Props) => {
  ...
  const handleSubmit = (values: any) => {
    ...
    // const convertBase64ToFile = (image: string) => {
    ...
    //   }
    //   const name = `${Math.random().toString(36).slice(-5)}.jpeg`;

    //   const newBlob = new File([ab], name, {
    //     type: 'image/jpeg'
    //   });
    //   return newBlob;
    // };
      ...
      tab: (
        <Grid container spacing={3}>
          <Grid item xs={12} lg={4}>
            <TextFieldCustom name="name" formik={formik} errors={errors} label="Tên website" required />
          </Grid>

          <Grid item xs={12} lg={4}>
            <WebsiteGroupSelect formik={formik} setFieldValue={formik.setFieldValue} addOrEdit={true} />
          </Grid>
          ...
          <Grid item xs={12} lg={8}>
            <TextFieldCustom name="description" formik={formik} errors={errors} label="Mô tả" multiline rows={6} />
          </Grid>

          <Grid item xs={12} lg={4}>
            <UploadImage
              image={formik.values.logo ? `${process.env.REACT_APP_SERVER}${formik.values.logo}` : ''}
  ...
  return (
    <>
      <DialogPopUp
        open={visible}
        title={dataEdit?.id ? `Cập nhật thông tin ${dataEdit?.name}` : 'Thêm mới wesite'}
        handleClose={() => {
          closePopUp();
        }}
        styleBox={{
