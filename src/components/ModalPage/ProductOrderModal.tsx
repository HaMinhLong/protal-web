...
const ProductOrderModal = ({ open, dataEdit, handleClose, formikProp }: Props) => {
  const [dataProductOrder, setDataProductOrder] = useState<ProductOrderType>(initialData);
  ...
  const validationSchema = yup.object().shape({
    ...
    amount: yup.number().min(1, 'Vui lòng nhập số lượng lớn hơn 0').required('Vui lòng nhập số lượng'),
    price: yup.number().min(1, 'Vui lòng nhập giá tiền lớn hơn 0').required('Vui lòng nhập giá tiền'),
    negotiablePrice: yup.number().min(1, 'Vui lòng nhập giá thỏa thuận lớn hơn 0').required('Vui lòng nhập số lượng')
  });
  ...
  const handleSubmit = (values: ProductOrderType) => {
    if (values.id) {
      ...
      if (
        formikProp?.values?.productOrders?.find((item) => item.productName === values.productName && item.flag !== 'delete') &&
        !values.id
      ) {
        createNotification('error', 'Sản phẩm đã được thêm vào giỏ hàng trước đó!');
        return;
      }
      const newData = [{ ...values, id: Math.floor(Math.random() * 100000) }].concat(formikProp.values?.productOrders);
  ...
  return (
    <DialogPopUp
      open={open}
      title={dataProductOrder?.id ? `Cập nhật thông tin ${dataProductOrder?.productName}` : 'Thêm mới sản phẩm'}
      handleClose={() => {
        closePopUp();
      }}
      ...
      <Box>
        <form onSubmit={formik.handleSubmit}>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12} md={6} lg={6}>
              <ProductSelect
                ...
                handleChange={(value) => {
                  ...
                  formik.setFieldValue('negotiablePrice', value.negotiablePrice || 0);
                }}
                websiteId={formikProp?.values?.websiteId}
              />
            </Grid>
            <Grid item xs={12} md={6} lg={6}>
              <TextFieldCustom name="category" formik={formik} required label="Loại sản phẩm" />
            </Grid>
            <Grid item xs={12} md={6} lg={6}>
              <TextFieldCustom
                name="amount"
                ...
                label="Số lượng"
                type="number"
                handleChange={(value) => handleChangeTotalPrice(Number(value), formik?.values?.negotiablePrice)}
              />
            </Grid>
            <Grid item xs={12} md={6} lg={6}>
              <TextFieldCustom name="price" formik={formik} required label="Giá tiền" type="number" readOnly />
            </Grid>
            <Grid item xs={12} md={6} lg={6}>
              <TextFieldCustom
            ...
            <Grid item xs={12} md={6} lg={6}>
              <TextFieldCustom name="totalPrice" formik={formik} required label="Tổng tiền" readOnly />
            </Grid>
          </Grid>
          <Box
            sx={{
