...
// TYPES IMPORT
import { PaymentMethodType, ResponseError } from 'types/paymentMethod';
...
const PaymentMethodDrawer = ({ visible, closeDrawer, dataEdit, getList }: Props) => {
  ...
  const validationSchema = yup.object().shape({
    name: yup.string().trim().max(50).required('Vui lòng nhập tên phương thức thanh toán'),
      ...
      <Drawer anchor={'right'} open={visible} onClose={changeDrawer}>
        ...
        >
          <Typography variant="h4" sx={{ mb: 3 }}>
            {dataEdit?.id ? `Cập nhật thông tin ${dataEdit?.name}` : 'Thêm mới phương thức thanh toán'}
            <Divider sx={{ mt: 1 }} />
          </Typography>
          <form onSubmit={formik.handleSubmit}>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <TextFieldCustom name="name" formik={formik} errors={errors} label="Tên phương thức thanh toán" />
              </Grid>
